...
class Network(tf.keras.Model):
    ...
    def __init__(self, batch_size=1):
        ...
        with tf.name_scope("worker_critic"):
            self.mlp1_worker_v = tf.keras.layers.Dense(units=128, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
            self.mlp2_worker_v = tf.keras.layers.Dense(units=128, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
            self.mlp3_worker_v = tf.keras.layers.Dense(units=128, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
            self.worker_v = tf.keras.layers.Dense(units=1, kernel_initializer=normc_initializer(1.0))
        with tf.name_scope("manager_critic"):
            self.mlp1_manager_v = tf.keras.layers.Dense(units=128, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
            self.mlp2_manager_v = tf.keras.layers.Dense(units=128, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
            self.mlp3_manager_v = tf.keras.layers.Dense(units=128, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
            self.manager_v = tf.keras.layers.Dense(units=1, kernel_initializer=normc_initializer(1.0))

    def get_neglogp(self, logits, action):
        return self.pd.neglogp(logits, action)

    @tf.function
    def get_worker_v(self, worker_obs):
        worker_v = self.mlp1_worker_v(worker_obs)
        worker_v = self.mlp2_worker_v(worker_v)
        worker_v = self.mlp3_worker_v(worker_v)
        worker_v = self.worker_v(worker_v)[:, 0]
        return worker_v
    
    @tf.function
    def get_manager_v(self, manager_obs):
        manager_v = self.mlp1_manager_v(manager_obs)
        manager_v = self.mlp2_manager_v(manager_v)
        manager_v = self.mlp3_manager_v(manager_v)
        manager_v = self.manager_v(manager_v)[:, 0]
        return manager_v

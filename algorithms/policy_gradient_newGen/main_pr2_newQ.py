...
from network_pr2_newQ import Network
...
def actor(n_agents=2):
    ...
    network.get_jq_value(np.array([obs[0]]), np.array([[0, 0, 0, 0, 0]]).astype(np.float32))
    # Get agent type
    ...
    while True:
        ...
        mb_jqvalues = np.zeros([nsteps, n_agents, 5], dtype=np.float32)
        ...
        for step in range(nsteps):
            ...
            for ai in range(n_agents):
                other_ai = 1 - ai
                jq_value = np.array([0.0 for _ in range(5)])
                ...
                for idx in range(value_n_particles):
                    jq_value += network.get_jq_value(np.array([obs_dc[ai]]), opponent_actions[:, idx])
        ...
        for ai in range(n_agents):
            ...
            for t in reversed(range(nsteps)):
                if t == nsteps - 1:
                    ...
                    obs_dc = deepcopy(obs)
                    last_jq_value = np.array([0.0 for _ in range(5)])
                    actions, neglogp, entropy, probs, p = network(np.array([obs_dc[ai]]))
                    opponent_actions = network.get_ocp(np.array([obs_dc[ai]]), np.array([actions[0]]), value_n_particles)
                    for idx in range(value_n_particles):
                        last_jq_value += network.get_jq_value(np.array([obs_dc[ai]]), opponent_actions[:, idx])
                    last_jq_value /= value_n_particles
                    nextvalues = 0
                    for idx in range(5):
                        nextvalues += last_jq_value[0, idx] * probs[0, idx]
                else:
                    nextnonterminal = 1.0 - mb_dones[t+1, ai]
                    nextvalues = 0
                    for idx in range(5):
                        nextvalues += mb_jqvalues[t+1, ai, idx] * mb_qprob[t+1, ai, idx]
                for idx in range(5):
                    mb_values[t, ai] += mb_jqvalues[t, ai, idx] * mb_qprob[t, ai, idx]
...
def learner():
    ...
    def ocp_loss(b_obs, b_actions):
        ...
        for idx in range(n_fixed_actions):
            svgd_target_value = network.get_jq_value(b_obs, fixed_actions[:, idx])
            batch_idx = tf.range(b_obs.shape[0])
            batch_idx = tf.expand_dims(batch_idx, axis=-1)
            gather_actions = tf.concat([batch_idx, tf.expand_dims(b_actions, axis=-1)], axis=-1)
            svgd_target_value = tf.gather_nd(svgd_target_value, gather_actions)
    ...
    def q_loss(b_obs, b_next_obs, b_actions, b_next_actions, b_other_actions, b_qdones, b_rewards):
        ...
        batch_idx = tf.expand_dims(batch_idx, axis=-1)
        for idx in range(value_n_particles):
            q_value_target = network.get_jq_value(b_next_obs, opponent_target_actions[:, idx])
            q_value_target = tf.stop_gradient(q_value_target)
            gather_next_actions = tf.concat([batch_idx, tf.expand_dims(b_next_actions, axis=-1)], axis=-1)
            q_value_target = tf.gather_nd(q_value_target, gather_next_actions)
            q_value_targets.append(q_value_target)
            q_value = network.get_jq_value(b_obs, tf.dtypes.cast(b_other_actions[:, idx], tf.float32))
            gather_actions = tf.concat([batch_idx, tf.expand_dims(b_actions, axis=-1)], axis=-1)
            q_value = tf.gather_nd(q_value, gather_actions)
            q_values.append(q_value)
        q_value_targets = tf.stack(q_value_targets, axis=1)
        q_value_targets = tf.stop_gradient(q_value_targets)

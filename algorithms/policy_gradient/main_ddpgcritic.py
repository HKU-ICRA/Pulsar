...
from network_ddpgcritic import Network
    ...
    while True:
        ...
        for step in range(nsteps):
            ...
            for ai in range(n_agents):
                ...
                if ai == 0:
                    ...
                    mb_qvalues[step] = network.get_q_value(np.array([obs_dc[0]]), np.array([actions[0]]))
                    mb_tqvalues[step] = network.get_tq_value(np.array([obs_dc[0]]), np.array([t_actions[0]]))
        ...
        for t in reversed(range(nsteps)):
            if t == nsteps - 1:
                ...
                target_q = network.get_tq_value(np.array([obs_dc[0]]), np.array([t_actions[0]]))
...
def learner():
    ...
    @tf.function
    def ppo_loss(b_obs, b_actions, b_returns, b_dones, b_qvalues, b_neglogpacs, b_qprob, b_all_qvalues, b_advs, b_rewards, b_vreturns, b_actualvalues):
        ...
        for _ in range(noptepochs):
            ...
            for start in range(0, nbatch, nbatch_steps):
                ...
                with tf.GradientTape() as tape:
                    ...
                    vpred = network.get_q_value(mb_obs, mb_actions)

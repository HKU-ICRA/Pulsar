...
from network_ddpgcentcritic import Network
...
def actor(n_agents=4):
    ...
    network.get_q_value(np.array([obs[0]]), np.array([[0, 0, 0, 0, 0]]))
    network.get_tq_value(np.array([obs[0]]), np.array([[0, 0, 0, 0, 0]]))
    # Get agent type
    ...
    while True:
        ...
        mb_actualvalues = np.zeros([nsteps, 1], dtype=np.float32)
        mb_tqvalues = np.zeros([nsteps, 1], dtype=np.float32)
        mb_besta_tqvalues = np.zeros([nsteps, 1], dtype=np.float32)
        ...
        mb_qtprob = np.zeros([nsteps, 5], dtype=np.float32)
        ...
        for step in range(nsteps):
            ...
            for ai in range(n_agents):
                ...
                if ai == 0:
                    t_actions, t_probs = network.get_tp(np.array([obs_dc[0]]))
                    mb_actualvalues[step] = network.get_value(np.array([obs_dc[0]]))
                    #mb_actualvalues[step] = network.get_value(np.array([obs_dc[0]]), probs)
                    mb_neglogpacs[step] = neglogp
                    mb_obs[step] = obs_dc[0]
                    mb_dones[step] = dones
                    mb_actions[step] = actions
                    mb_qprob[step] = probs
                    mb_qtprob[step] = t_probs
            mb_qvalues[step] = network.get_q_value(np.array([obs_dc[0]]), np.expand_dims(np.array(agent_actions), axis=0))
            mb_tqvalues[step] = network.get_tq_value(np.array([obs_dc[0]]), np.expand_dims(np.array(agent_actions), axis=0))
        ...
        mb_vreturns = np.zeros_like(mb_rewards)
        ...
        for t in reversed(range(nsteps)):
            if t == nsteps - 1:
                ...
                for ai in range(n_agents):
                    obs_dc = deepcopy(obs)
                    t_actions, _ = network.get_tp(np.array([obs_dc[0]]))
                    agent_tactions.append(t_actions[0])
                target_q = network.get_tq_value(np.array([obs_dc[0]]), np.expand_dims(np.array(agent_tactions), axis=0))
                last_values = network.get_value(np.array([obs_dc[0]]))
            else:
                nextnonterminal = 1.0 - mb_dones[t+1]
                target_q = mb_tqvalues[t+1]
                last_values = mb_actualvalues[t+1]
            mb_returns[t] = mb_rewards[t] + nextnonterminal * gamma * target_q
            mb_vreturns[t] = mb_rewards[t] + nextnonterminal * gamma * last_values
        for t in range(nsteps):
            #valuef = network.get_value(mb_obs[t:t+1], mb_qtprob[t:t+1])
            mb_advs[t] = mb_qvalues[t] - mb_actualvalues[t]
        ...
        mb_vreturns = np.squeeze(mb_vreturns, axis=-1)
        mb_actualvalues = np.squeeze(mb_actualvalues, axis=-1)
        trajectory = {
                    'mb_obs': mb_obs,
                    ...
                    'mb_vreturns': mb_vreturns,
                    'mb_actualvalues': mb_actualvalues,
...
def learner():
    ...
    #@tf.function
    def ppo_loss(b_obs, b_actions, b_returns, b_dones, b_qvalues, b_neglogpacs, b_qprob, b_all_qvalues, b_advs, b_rewards, b_vreturns, b_actualvalues, b_all_actions):
        ...
        for _ in range(noptepochs):
            ...
            for start in range(0, nbatch, nbatch_steps):
                ...
                mb_vreturns = tf.gather(b_vreturns, mbinds)
                mb_actualvalues = tf.gather(b_actualvalues, mbinds)
                ...
                with tf.GradientTape() as tape:
                    ...
                    vf_loss = tf.reduce_mean(tf.square(vpred - mb_returns))
                    # Calc value l
                    actualvpred = network.get_value(mb_obs)
                    vpredclipped = mb_actualvalues + tf.clip_by_value(actualvpred - mb_actualvalues, -CLIPRANGE, CLIPRANGE)
                    # Unclipped value
                    vf_losses1 = tf.square(actualvpred - mb_vreturns)
                    # Clipped value
                    vf_losses2 = tf.square(vpredclipped - mb_vreturns)
                    actualvf_loss = .5 * tf.reduce_mean(tf.maximum(vf_losses1, vf_losses2))
                    ...
                    # Total loss
                    loss = pg_loss - entropy * ent_coef + vf_loss * vf_coef + actualvf_loss * vf_coef
    ...
    while True:
        trajectory = {
                    ...
                    'mb_vreturns':                       np.empty((nbatch,),         dtype=np.float32),
                    'mb_actualvalues':                   np.empty((nbatch,),         dtype=np.float32),
        ...
        b_vreturns = trajectory['mb_vreturns']
        b_actualvalues = trajectory['mb_actualvalues']
        ...
        losses, approxkls, entropies = ppo_loss(b_obs, b_actions, b_returns, b_dones,
                                                ...
                                                mb_rewards, b_vreturns, b_actualvalues,
                                                b_all_actions)
        network.polyak_qnet()
        network.polyak_pnet()

...
class Network(tf.keras.Model):
    ...
    @tf.function
    def get_tq_value(self, obs, action, other_action):
        action_1H = tf.one_hot(action, 5, dtype=tf.float32)
        other_action_1H = tf.one_hot(other_action, 5, dtype=tf.float32)
        tq_input = tf.concat([obs, action_1H, other_action_1H], axis=-1)    

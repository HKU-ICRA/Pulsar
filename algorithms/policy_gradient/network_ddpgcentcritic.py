...
class Network(tf.keras.Model):
    ...
    def get_q_value(self, obs, action):
        oh_actions = np.zeros([action.shape[0], 4 * 5])
        for a in range(4):
            action_1H = tf.one_hot(action[:, a], 5, dtype=tf.float32)
            oh_actions[:, a*5:(a*5)+5] = np.array(action_1H)
        q_input = tf.concat([obs, oh_actions], axis=-1)    
    ...
    def get_tq_value(self, obs, action):
        ...
        for a in range(4):
            action_1H = tf.one_hot(action[:, a], 5, dtype=tf.float32)
            oh_actions[:, a*5:(a*5)+5] = np.array(action_1H)
        tq_input = tf.concat([obs, oh_actions], axis=-1)    

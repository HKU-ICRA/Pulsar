...
from network_manormal import Network
...
def actor(n_agents=2):
    ...
    while True:
        ...
        for step in range(nsteps):
            ...
            obs_dc = deepcopy(obs)
            agent_actions, neglogp, entropy, value, probs, p = network(np.array(obs_dc))
            mb_values[step] = np.expand_dims(value, axis=-1)
            mb_neglogpacs[step] = np.expand_dims(neglogp, axis=-1)
            mb_dones[step] = dones
            mb_actions[step] = agent_actions
            mb_obs[step] = obs_dc

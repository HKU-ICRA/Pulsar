...
from network_gcn import Network
...
def actor(n_agents=4):
    ...
    while True:
        ...
        mb_all_obs = np.zeros([nsteps, n_agents, n_agents, 4 * n_agents], dtype=np.float32)
        ...
        for ai in range(n_agents):
            for t in reversed(range(nsteps)):
                if t == nsteps - 1:
                    ...
                    nextvalues = mb_qvalues[t+1, ai]
                delta = mb_rewards[t, ai] + gamma * nextvalues * nextnonterminal - mb_qvalues[t, ai]
                mb_advs[t, ai] = lastgaelam = delta + gamma * lam * nextnonterminal * lastgaelam
            mb_returns[:, ai] = mb_advs[:, ai] + mb_qvalues[:, ai]
        # Flatten obs
        mb_all_obs = np.concatenate(np.swapaxes(mb_all_obs, 0, 1), axis=0)
        mb_obs = np.concatenate(np.swapaxes(mb_obs, 0, 1), axis=0)
        mb_actions = np.concatenate(np.swapaxes(mb_actions, 0, 1), axis=0)
        mb_returns = np.concatenate(np.swapaxes(mb_returns, 0, 1), axis=0)
        mb_dones = np.concatenate(np.swapaxes(mb_dones, 0, 1), axis=0)
        mb_qvalues = np.concatenate(np.swapaxes(mb_qvalues, 0, 1), axis=0)
        mb_neglogpacs = np.concatenate(np.swapaxes(mb_neglogpacs, 0, 1), axis=0)
        mb_rewards = np.concatenate(np.swapaxes(mb_rewards, 0, 1), axis=0)
        mb_qprob = np.concatenate(np.swapaxes(mb_qprob, 0, 1), axis=0)
        mb_all_qvalues = np.concatenate(np.swapaxes(mb_all_qvalues, 0, 1), axis=0)
...
def learner():
    ...
    @tf.function
    def ppo_loss(b_all_obs, b_obs, b_actions, b_returns, b_dones, b_qvalues, b_neglogpacs, b_qprob, b_all_qvalues):
        ...
        for _ in range(noptepochs):
            ...
            for start in range(0, nbatch, nbatch_steps):
                ...
                mb_all_obs = tf.gather(b_all_obs, mbinds)
                ...
                with tf.GradientTape() as tape:
                    p_actions, p_neglogp, p_entropy, vpred, p_probs, p_p, taken_action_neglogp = network(mb_all_obs, 0, [mb_actions])
    ...
    while True:
        trajectory = {
                    'mb_all_obs':                        np.empty((nbatch, 4, 16),     dtype=np.float32),
        ...
        b_all_obs = trajectory['mb_all_obs']
        ...
        losses, approxkls, entropies = ppo_loss(b_all_obs, b_obs, b_actions, b_returns, b_dones, b_qvalues, b_neglogpacs, b_qprob, b_all_qvalues)

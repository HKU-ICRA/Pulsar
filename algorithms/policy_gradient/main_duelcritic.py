...
from network_duelcritic import Network
    ...
    while True:
        ...
        for step in range(nsteps):
            ...
            for ai in range(n_agents):
                ...
                if ai == 0:
                    t_actions = network.get_tp(np.array([obs_dc[0]]))
        ...
        for t in reversed(range(nsteps)):
            ...
            mb_returns[t] = mb_rewards[t] + nextnonterminal * 0.99 * target_q
        for t in range(nsteps):
            mb_advs[t] = network.get_a_value(mb_obs[t:t+1], mb_actions[t:t+1].astype(np.int32))
        ...
        for _ in range(noptepochs):
            ...
            for start in range(0, nbatch, nbatch_steps):
                ...
                with tf.GradientTape() as tape:
                    ...
                    vf_loss = tf.reduce_mean(tf.square(vpred - mb_returns))
                    # Calc value l
                    #actualvpred = network.get_value(mb_obs, mb_qprob)

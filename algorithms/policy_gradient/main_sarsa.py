...
from network_sarsa import Network
        ...
        for ai in range(n_agents):
            for t in reversed(range(nsteps)):
                if t == nsteps - 1:
                    ...
                    obs_dc = deepcopy(obs)
                    actions, neglogp, entropy, probs, p = network(np.array([obs_dc[ai]]))
                    target_q = network.get_q_value(np.array([obs_dc[ai]]), np.array([actions[0]]))
                    last_values = network.get_value(np.array([obs_dc[ai]]))
                else:
                    nextnonterminal = 1.0 - mb_dones[t+1, ai]
                    target_q = mb_qvalues[t+1, ai]
...
def learner():
    ...
    @tf.function
    def ppo_loss(b_obs, b_actions, b_returns, b_dones, b_qvalues, b_neglogpacs, b_qprob, b_advs, b_rewards, b_qtargets, b_values):
        ...
        for _ in range(noptepochs):
            ...
            for start in range(0, nbatch, nbatch_steps):
                ...
                with tf.GradientTape() as tape:
                    ...
                    # Calc q loss
                    qpred = network.get_q_value(mb_obs, mb_actions)

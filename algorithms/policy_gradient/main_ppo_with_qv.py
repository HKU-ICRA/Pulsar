...
from network_ppo_with_qv import Network
    ...
    while True:
        ...
        for step in range(nsteps):
            ...
            for ai in range(n_agents):
                ...
                mb_tqvalues[step, ai] = network.get_tq_value(np.array([obs_dc[ai]]), np.array([t_actions[0]]))
        ...
        for ai in range(n_agents):
            for t in reversed(range(nsteps)):
                if t == nsteps - 1:
                    ...
                    obs_dc = deepcopy(obs)
                    t_actions, _ = network.get_tp(np.array([obs_dc[ai]]))
                    target_q = network.get_tq_value(np.array([obs_dc[ai]]), np.array([t_actions[0]]))
                    last_values = network.get_value(np.array([obs_dc[ai]]))
                else:
                    nextnonterminal = 1.0 - mb_dones[t+1, ai]
                    target_q = mb_tqvalues[t+1, ai]
...
def learner():
    ...
    @tf.function
    def ppo_loss(b_obs, b_actions, b_returns, b_dones, b_qvalues, b_neglogpacs, b_qprob, b_all_qvalues, b_advs, b_rewards, b_qtargets, b_values):
        ...
        for _ in range(noptepochs):
            ...
            for start in range(0, nbatch, nbatch_steps):
                ...
                with tf.GradientTape() as tape:
                    ...
                    mb_q = network.get_q_value(mb_obs, mb_actions)
    ...
    while True:
        ...
        b_obs = trajectory['mb_obs'][:, 0]
        b_actions = trajectory['mb_actions'][:, 0]
        b_dones = trajectory['mb_dones'][:, 0]
        b_qvalues = trajectory['mb_qvalues'][:, 0]
        b_neglogpacs = trajectory['mb_neglogpacs'][:, 0]
        b_advs = trajectory['mb_advs'][:, 0]
        mb_rewards = trajectory['mb_rewards'][:, 0]
        b_qprob = trajectory['mb_qprob'][:, 0]
        b_all_qvalues = trajectory['mb_all_qvalues'][:, 0]
        b_returns = trajectory['mb_returns'][:, 0]
        b_qtargets = trajectory['mb_qtargets'][:, 0]
        b_values = trajectory['mb_values'][:, 0]

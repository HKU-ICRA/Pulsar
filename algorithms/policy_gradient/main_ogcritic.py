...
from network_ogcritic import Network
    ...
    while True:
        ...
        for step in range(nsteps):
            ...
            for ai in range(n_agents):
                ...
                if ai == 0:
                    ...
                    mb_tqvalues[step] = network.get_tq_value(np.array([obs_dc[0]]), np.array([actions[0]]))
        ...
        for t in reversed(range(nsteps)):
            if t == nsteps - 1:
                ...
                obs_dc = deepcopy(obs)
                actions, neglogp, entropy, probs, p = network(np.array([obs_dc[ai]]))
                target_q = network.get_tq_value(np.array([obs_dc[0]]), np.array([actions[0]]))
            else:
        ...
        for t in reversed(range(nsteps)):
            valuef = 0
            for ai in range(5):
                valuef += network.get_q_value(mb_obs[t:t+1], np.array([ai])) * mb_qprob[t, ai]

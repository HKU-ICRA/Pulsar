...
def actor(n_agents=4):
    ...
    while True:
        ...
        mb_all_qvalues = np.zeros([nsteps, 4], dtype=np.float32)
        mb_neglogpacs = np.zeros([nsteps, 4], dtype=np.float32)
        mb_dones = np.zeros([nsteps,], dtype=np.float32)
        mb_actions = np.zeros([nsteps,], dtype=np.float32)
        mb_qprob = np.zeros([nsteps, 4], dtype=np.float32)
        ...
        for step in range(nsteps):
            ...
            for ai in range(n_agents):
                ...
                if ai == 0:
                    ...
                    mb_qprob[step] = probs
                    for qi in range(4):
                        mb_neglogpacs[step][qi] = network.get_neglogp(p, np.array([qi]))
                        mb_all_qvalues[step][qi] = network.get_q_value(np.array([obs_dc[ai]]), np.array([qi]))
        ...
        obs_dc = deepcopy(obs)
        _, _, _, _, q_prob, _ = network(np.array([obs_dc[0]]))
        ...
        for t in reversed(range(nsteps)):
            if t == nsteps - 1:
                ...
                if not dones:
                    for act in range(4):
                        obs_dc = deepcopy(obs)
                        sample_q = network.get_q_value(np.array([obs_dc[0]]), np.array([act]))
                        nextvalues += q_prob[0, act] * sample_q
            mb_returns[t] = mb_rewards[t] + lam * nextvalues
...
def learner():
        ...
        for _ in range(noptepochs):
            ...
            for start in range(0, actual_nbatch, nbatch_steps):
                ...
                with tf.GradientTape() as tape:
                    p_actions, p_neglogp, p_entropy, vpred, p_states, p_prev_states, taken_action_neglogp = pulsar(mb_scalar_features,
                                                                                                                        mb_entity_masks,
                                                                                                                        mb_entities,
                                                                                                                        mb_baselines,
                                                                                                                        mb_states,
    ...
    while True:
        trajectory = {
                    ...
                    'mb_actions':                        np.empty((nbatch,),     dtype=np.float32),
                    ...
                    'mb_neglogpacs':                     np.empty((nbatch, 4),         dtype=np.float32),
                    'mb_rewards':                        np.empty((nbatch),         dtype=np.float32),
                    'mb_qprob':                          np.empty((nbatch, 4),         dtype=np.float32),
                    'mb_all_qvalues':                    np.empty((nbatch, 4),         dtype=np.float32)
                    }
        ...
        losses, approxkls, entropies = ppo_loss(b_obs, b_actions, b_returns, b_dones, b_qvalues, b_neglogpacs, b_qprob, b_all_qvalues)

...
from network_td3critic import Network
...
def actor(n_agents=4):
    ...
    while True:
        ...
        for t in reversed(range(nsteps)):
            valuef = 0
            for ai in range(5):
                valuef += network.get_tq_value(mb_obs[t:t+1], np.array([ai])) * mb_qtprob[t, ai]
            mb_advs[t] = mb_qvalues[t] - valuef
...
def learner():
    ...
    @tf.function
    def ppo_loss(b_obs, b_actions, b_returns, b_dones, b_qvalues, b_neglogpacs, b_qprob, b_all_qvalues, b_advs, b_rewards):

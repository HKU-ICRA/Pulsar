...
class Network(tf.keras.Model):
    ...
    def __init__(self, batch_size=1):
        ...
        with tf.name_scope("ivalue"):
            self.mlp1_iv = tf.keras.layers.Dense(units=128, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
            self.mlp2_iv = tf.keras.layers.Dense(units=128, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
            self.mlp3_iv = tf.keras.layers.Dense(units=128, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
            self.iv = tf.keras.layers.Dense(units=1, kernel_initializer=normc_initializer(1.0))
        self.time_embd = tf.keras.layers.Dense(units=128, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
        with tf.name_scope("rew"):
            self.state_fe1 = tf.keras.layers.Dense(units=32, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
            self.state_fe2 = tf.keras.layers.Dense(units=32, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
            self.mlp1_rew = tf.keras.layers.Dense(units=128, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
            self.mlp2_rew = tf.keras.layers.Dense(units=1, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
        with tf.name_scope("dependency"):
            self.mlp1_d = tf.keras.layers.Dense(units=128, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
            self.mlp2_d = tf.keras.layers.Dense(units=128, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
            self.ff1_d = tf.keras.layers.Dense(units=128, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
            self.d = tf.keras.layers.Dense(units=5, activation=tf.nn.tanh, kernel_initializer=normc_initializer(0.01))
    ...
    @tf.function
    def get_ivalue(self, obs, action):
        action_1H = tf.one_hot(action, 5, dtype=tf.float32)
        iv_input = tf.concat([obs, action_1H], axis=-1)    
        with tf.name_scope("ivalue"):
            iv = self.mlp1_iv(iv_input)
            iv = self.mlp2_iv(iv)
            iv = self.mlp3_iv(iv)
            iv = self.iv(iv)[:, 0]
        return iv
    ...
    @tf.function
    def get_rew(self, obs1, obs2, rew, adv, k):
        st1 = self.state_fe1(obs1)
        st1 = self.state_fe2(st1)
        st2 = self.state_fe1(obs2)
        st2 = self.state_fe2(st2)
        features = tf.concat([adv, rew, st1, st2], axis=-1)
        features = self.mlp1_rew(features)
        time_embd = self.time_embd(k)
        outputs = features * time_embd
        outputs = self.mlp2_rew(outputs)
        return outputs

    @tf.function
    def get_dependency(self, obs1, obs2, k):
        d1 = self.mlp1_d(obs1)
        d1 = self.mlp2_d(d1)
        d2 = self.mlp1_d(obs2)
        d2 = self.mlp2_d(d2)
        ds = tf.concat([d1, d2], axis=-1)
        ds = self.ff1_d(ds)
        time_embd = self.time_embd(k)
        ds = ds * time_embd
        d = self.d(ds)

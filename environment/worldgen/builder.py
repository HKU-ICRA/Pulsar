...
import numpy as np
from mujoco_py import const, MjSim, load_model_from_path
import os
...
from mujoco_worldgen.util.sim_funcs import joint_qpos_idxs
...
class WorldBuilder(Obj):
    ...
    def __init__(self, world_params, meshdir, texturedir, seed, env_no):
        self.env_no = env_no
    ...
    def get_sim(self):
        ...
        modelxml = "./model" + str(self.env_no) + ".xml"
        xmlfile = open(modelxml, "w") 
        xmlfile.write(xml)
        xmlfile.close()
        model = load_model_from_path(modelxml)
        os.remove(modelxml)
        
        sim = MjSim(model, nsubsteps=self.world_params.num_substeps)

        for name, value in xinit_dict.items():
            idx = joint_qpos_idxs(sim, name)
            sim.data.qpos[idx] = value

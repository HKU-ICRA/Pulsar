...
while True:
    ...
    mb_returns = trajectory['mb_returns'][idxs]
    ...
    if mb_states != None:
        for i in range(len(mb_states)):
            for j in range(len(mb_states[i])):
                mb_states[i][j] = tf.convert_to_tensor(np.repeat(mb_states[i][j][0:1][:], repeats=n_batch, axis=0))

    inds = np.arange(n_batch)
    for _ in range(noptepochs):
        np.random.shuffle(inds)
        
        mb_scalar_features['match_time'] = mb_scalar_features['match_time'][inds]
        mb_scalar_features['bptt_match_time'] = mb_scalar_features['bptt_match_time'][inds]
        mb_entity_masks = trajectory['mb_entity_masks'][inds]
        mb_baselines = trajectory['mb_baselines'][inds]
        mb_actions_xy = trajectory['mb_actions_xy'][inds]
        mb_actions_yaw = trajectory['mb_actions_yaw'][inds]
        mb_returns = trajectory['mb_returns'][inds]
        mb_dones = trajectory['mb_dones'][inds]
        mb_values = trajectory['mb_values'][inds]
        mb_neglogpacs_xy = trajectory['mb_neglogpacs_xy'][inds]
        mb_neglogpacs_yaw = trajectory['mb_neglogpacs_yaw'][inds]
